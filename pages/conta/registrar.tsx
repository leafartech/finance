...
export default function Entrar() {
    ...
    async function formSubmited(data: any) {
        await axios.post('/api/user', { ...data, type: 'register' }).then((res) => {
            if (res.data.error) {
                ...
                <div className="w-full p-4 sm:max-w-sm">
                    <form className="flex flex-col items-center justify-center gap-4 text-zinc-300" onSubmit={handleSubmit(formSubmited)}>
                        ...
                        <div className="flex flex-col gap-2 w-full">
                            ...
                            <input type="text" id="name" {...register('name')} className="flex-1 bg-transparent px-2 py-2 rounded-md border border-zinc-600 outline-none focus:border-gray-500" />
                        ...
                        <div>
                            <p>Já possui uma conta? <Link href="/conta/entrar" className="text-emerald-500 font-medium">Fazer login</Link></p>
                        </div>
                    </form>
                </div>
            </Template>
